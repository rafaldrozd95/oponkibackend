...
module.exports = class Przelewy24 {
  ...
  setSessionId(value) {
    if (value.length === 0 || value.length > 100) {
      throw new Error(
        "p24_session_id value must have from 0 to 100 characters."
      );
    }
  ...
  setDescription(value) {
    if (value.length === 0 || value.length > 1024) {
      throw new Error(
        "p24_description value must have from 0 to 1024 characters."
      );
    }
  ...
  setUrlReturn(value) {
    if (value.length === 0 || value.length > 250) {
      throw new Error(
        "p24_url_return value must have from 0 to 250 characters."
      );
    }
  ...
  setUrlStatus(value) {
    if (value.length === 0 || value.length > 250) {
      throw new Error(
        "p24_url_status value must have from 0 to 250 characters."
      );
    }
  ...
  getCrc() {
    const data =
      this.form.p24_session_id +
      "|" +
      this.form.p24_pos_id +
      "|" +
      this.form.p24_amount +
      "|" +
      this.form.p24_currency +
      "|" +
      this.salt;
  ...
  register() {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const rawResponse = await request.post(this.host + "trnRegister", {
          form: this.form,
        });
        ...
        if (rawResponse) {
          ...
          throw new Error(
            "Error happened while registering payment, please try again."
          );
        }
      } catch (e) {
        return reject(e);
  ...
  verify(sign) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const rawResponse = await request.post(this.host + "trnVerify", {
          form: this.form,
        });
        ...
        if (rawResponse) {
          ...
          throw new Error(
            "Error happened while verifying payment, please try again."
          );
        }
      } catch (e) {
        return reject(e);
